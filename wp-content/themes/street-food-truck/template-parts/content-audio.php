...
<?php
  ...
  $street_food_truck_audio = false;

  // Only get audio from the content if a playlist isn't present.
  if ( false === strpos( $content, 'wp-playlist-script' ) ) {
    $street_food_truck_audio = get_media_embedded_in_content( $content, array( 'audio' ) );
  }
?>
  ...
  <div class="post-main-box p-3 mb-3 wow zoomIn" data-wow-duration="2s">
      <?php
        if ( ! is_single() ) {
          ...
          if ( ! empty( $street_food_truck_audio ) ) {
            foreach ( $street_food_truck_audio as $street_food_truck_audio_html ) {
              echo '<div class="entry-audio">';
                echo $street_food_truck_audio_html;
