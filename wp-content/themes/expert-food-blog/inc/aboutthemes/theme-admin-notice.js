jQuery(document).ready(function($) {
    $('.notice[data-notice="get_started"]').on('click', '.notice-dismiss', function() {
        ...
        $.ajax({
            type: 'POST',
            data: {
                action: 'expert_food_blog_dismiss_notice',
            },
            url: ajaxurl,
...
(function($) {
    $(document).on('click', '#expert-food-blog-import-button', function(e) {
        e.preventDefault();
        const redirectUrl = $(this).attr('href');
        // Send AJAX request to dismiss the notice
        $.post(ajaxurl, { action: 'expert_food_blog_dismiss_notice' }, function() {
