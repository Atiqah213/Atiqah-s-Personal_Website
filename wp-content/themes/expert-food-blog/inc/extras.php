 ...
 * @return array
 */
function expert_food_blog_body_classes( $classes ) {
...
add_filter( 'body_class', 'expert_food_blog_body_classes' );
 ...
 */
 ...
 if ( ! function_exists( 'expert_food_blog_breadcrumb_title' ) ) {
    function expert_food_blog_breadcrumb_title() {
        ...
        elseif ( is_day() ) : 
            ...
            printf( __( 'Daily Archives: %s', 'expert-food-blog' ), esc_html( get_the_date() ) );
        
        elseif ( is_month() ) :
            /* translators: %s: Monthly Archives. */
            printf( __( 'Monthly Archives: %s', 'expert-food-blog' ), esc_html( get_the_date( 'F Y' ) ) );
        
        elseif ( is_year() ) :
            /* translators: %s: Yearly Archives. */
            printf( __( 'Yearly Archives: %s', 'expert-food-blog' ), esc_html( get_the_date( 'Y' ) ) );
            
        elseif ( is_category() ) :
            /* translators: %s: Category Archives. */
            printf( __( 'Category Archives: %s', 'expert-food-blog' ), esc_html( single_cat_title( '', false ) ) );

        elseif ( is_tag() ) :
            /* translators: %s: Tag Archives. */
            printf( __( 'Tag Archives: %s', 'expert-food-blog' ), esc_html( single_tag_title( '', false ) ) );
            
        elseif ( is_404() ) :
            printf( esc_html__( 'Error 404', 'expert-food-blog' ) );
            
        elseif ( is_author() ) :
            /* translators: %s: Author. */
            printf( __( 'Author: %s', 'expert-food-blog' ), esc_html( get_the_author() ) );        
...
function expert_food_blog_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','expert-food-blog'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','expert-food-blog'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','expert-food-blog'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
	$before		= '<li class="active">'; // Tag before the current Breadcrumb
	$after 		= '</li>'; // Tag after the current Breadcrumb
	$seprator	= get_theme_mod('expert_food_blog_breadcrumb_seprator','/');
	global $post;
	$homeLink = home_url();
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','expert-food-blog').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','expert-food-blog').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','expert-food-blog').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','expert-food-blog').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','expert-food-blog'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','expert-food-blog') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_filter( 'woocommerce_sale_flash', 'expert_food_blog_woocommerce_sale_flash_percentage', 10, 3 );

function expert_food_blog_woocommerce_sale_flash_percentage( $html, $post, $product ) {
	...
	$percentages = expert_food_blog_woocommerce_get_product_sale_percentages( $product );
	$label       = expert_food_blog_woocommerce_get_product_sale_percentage_label( $percentages, $original_text );
...
function expert_food_blog_woocommerce_get_product_sale_percentages( $product ) {
	$percentages = array(
		'min' => false,
		'max' => false,
	);
		...
		case 'grouped':
			...
			foreach ( $children as $child ) {
				if ( $child->is_purchasable() && ! $child->is_type( 'grouped' ) && $child->is_on_sale() ) {
					$child_percentage = expert_food_blog_woocommerce_get_product_sale_percentages( $child );
...
function expert_food_blog_woocommerce_get_product_sale_percentage_label( $percentages, $original_label ) {
	$label = '';
	...
	if ( ( empty( $percentages['min'] ) || empty( $percentages['max'] ) ) || ( $percentages['min'] === $percentages['max'] ) ) {
		/* translators: -%1$d%%: Discount. */
		$label = sprintf( _x( '-%1$d%%', 'product discount', 'expert-food-blog' ), max( $rounded_percentages ) );
	} else {
		/* translators: -%1$d%%: Discount. */
		/* translators: -%2$d%%: Discount. */
		$label = sprintf( _x( '-%1$d%% / -%2$d%%', 'product discount', 'expert-food-blog' ), $rounded_percentages['min'], $rounded_percentages['max'] );
	}

	$label = apply_filters( 'expert_food_blog_woocommerce_sale_flash_percentage_label', $label, $rounded_percentages, $percentages, $original_label );

	return $label;
}

function expert_food_blog_default_styles() {
    // Begin Style
    $css = '<style>';

    $logo_width = get_theme_mod('expert_food_blog_logo_width');
    $css .= '
...
add_action('wp_head', 'expert_food_blog_default_styles', 999); // Use a priority to ensure it loads after other styles