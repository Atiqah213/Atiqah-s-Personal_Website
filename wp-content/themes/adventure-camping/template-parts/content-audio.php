...
<?php
  ...
  $adventure_camping_audio = false;

  // Only get audio from the content if a playlist isn't present.
  if ( false === strpos( $content, 'wp-playlist-script' ) ) {
    $adventure_camping_audio = get_media_embedded_in_content( $content, array( 'audio' ) );
  }
?>
  ...
  <div class="post-main-box p-3 mb-3 wow zoomIn" data-wow-duration="2s">
      <?php
        if ( ! is_single() ) {
          ...
          if ( ! empty( $adventure_camping_audio ) ) {
            foreach ( $adventure_camping_audio as $adventure_camping_audio_html ) {
              echo '<div class="entry-audio">';
                echo $adventure_camping_audio_html;
